...
def get_detector(classes = Depends(get_classes)) -> Detector:
    return Detector(model_manager=_model_manager, classes=classes, ocr=ocr)
...
async def detect(
    ...
    model_name: str = Form(default="main"),
    text_detection: bool = Form(False),
    detector: Detector = Depends(get_detector),
):
    logging.info(f"/detect({img_file.filename=}, {model_name=}, {text_detection=})")
...
async def detect_batch(
    ...
    logging.info(f"/detect_batch({model_name=})")
    if not files:
...
async def detect_archive(
    ...
    logging.info(f"/detect_archive({model_name=})")
